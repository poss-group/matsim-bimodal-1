...
public class InsertionGenerator {
	...
	public static class Insertion {
		public final org.matsim.contrib.my_drt.optimizer.VehicleData.Entry vehicleEntry;
		public final InsertionPoint pickup;
		public final InsertionPoint dropoff;

		public Insertion(org.matsim.contrib.my_drt.passenger.DrtRequest request, org.matsim.contrib.my_drt.optimizer.VehicleData.Entry vehicleEntry, int pickupIdx, int dropoffIdx) {
	...
	public List<Insertion> generateInsertions(org.matsim.contrib.my_drt.passenger.DrtRequest drtRequest, org.matsim.contrib.my_drt.optimizer.VehicleData.Entry vEntry) {
		...
		for (int i = 0; i < stopCount; i++) {// insertions up to before last stop
			org.matsim.contrib.my_drt.optimizer.VehicleData.Stop nextStop = nextStop(vEntry, i);
	...
	private void generateDropoffInsertions(DrtRequest drtRequest, org.matsim.contrib.my_drt.optimizer.VehicleData.Entry vEntry, int i,
		...
		for (int j = i; j < stopCount; j++) {// insertions up to before last stop
			...
			if (j > i) {// no need to check the capacity constraints if i == j (already validated for `i`)
				org.matsim.contrib.my_drt.optimizer.VehicleData.Stop currentStop = currentStop(vEntry, j);
	...
	private org.matsim.contrib.my_drt.optimizer.VehicleData.Stop currentStop(org.matsim.contrib.my_drt.optimizer.VehicleData.Entry entry, int insertionIdx) {
		return entry.stops.get(insertionIdx - 1);
	}

	private org.matsim.contrib.my_drt.optimizer.VehicleData.Stop nextStop(VehicleData.Entry entry, int insertionIdx) {
