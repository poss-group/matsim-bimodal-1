 ...
 *
 */
package org.matsim.contrib.my_drt.optimizer.rebalancing.demandestimator;
...
public final class PreviousIterationDRTDemandEstimator implements ZonalDemandEstimator, PersonDepartureEventHandler {
	...
	private Map<Double, Map<org.matsim.contrib.my_drt.analysis.zonal.DrtZone, MutableInt>> currentIterationDepartures = new HashMap<>();
	private Map<Double, Map<org.matsim.contrib.my_drt.analysis.zonal.DrtZone, MutableInt>> previousIterationDepartures = new HashMap<>();
	...
	public void handleEvent(PersonDepartureEvent event) {
		if (event.getLegMode().equals(mode) || event.getLegMode().equals(drtSpeedUpMode)) {
			...
			org.matsim.contrib.my_drt.analysis.zonal.DrtZone zone = zonalSystem.getZoneForLinkId(event.getLinkId());
			if (zone == null) {
	...
	public ToDoubleFunction<org.matsim.contrib.my_drt.analysis.zonal.DrtZone> getExpectedDemandForTimeBin(double time) {
