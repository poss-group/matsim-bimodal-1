...
package org.matsim.contrib.my_drt.optimizer;

import com.google.common.collect.ImmutableList;
import org.matsim.contrib.my_drt.optimizer.VehicleData.Entry;
import org.matsim.contrib.my_drt.optimizer.VehicleData.EntryFactory;
import org.matsim.contrib.my_drt.optimizer.VehicleData.Stop;
...
public class VehicleDataEntryFactoryImpl implements EntryFactory {
	...
	public VehicleDataEntryFactoryImpl(org.matsim.contrib.my_drt.run.DrtConfigGroup drtCfg) {
		lookAhead = drtCfg.getMaxWaitTime() - drtCfg.getStopDuration();
		if (lookAhead < 0) {
			throw new IllegalArgumentException(
					org.matsim.contrib.my_drt.run.DrtConfigGroup.MAX_WAIT_TIME + " must not be smaller than " + DrtConfigGroup.STOP_DURATION);
		}
	}
	...
	public Entry create(DvrpVehicle vehicle, double currentTime) {
		...
		if (schedule.getStatus() == ScheduleStatus.STARTED) {
			...
			switch (org.matsim.contrib.my_drt.schedule.DrtTaskBaseType.getBaseType(startTask)) {
				case DRIVE:
					org.matsim.contrib.my_drt.schedule.DrtDriveTask driveTask = (DrtDriveTask)startTask;
				...
				case STOP:
					org.matsim.contrib.my_drt.schedule.DrtStopTask stopTask = (org.matsim.contrib.my_drt.schedule.DrtStopTask)startTask;
					start = new LinkTimePair(stopTask.getLink(), stopTask.getEndTime());
					break;

				case STAY:
					org.matsim.contrib.my_drt.schedule.DrtStayTask stayTask = (DrtStayTask)startTask;
		...
		List<org.matsim.contrib.my_drt.schedule.DrtStopTask> stopTasks = new ArrayList<>();
