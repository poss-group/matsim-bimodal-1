...
package org.matsim.contrib.my_drt.optimizer.rebalancing;
...
public class RebalancingUtils {
	public static Map<org.matsim.contrib.my_drt.analysis.zonal.DrtZone, List<DvrpVehicle>> groupRebalancableVehicles(org.matsim.contrib.my_drt.analysis.zonal.DrtZonalSystem zonalSystem,
                                                                                                                     RebalancingParams params, Stream<? extends DvrpVehicle> rebalancableVehicles, double time) {
		Map<org.matsim.contrib.my_drt.analysis.zonal.DrtZone, List<DvrpVehicle>> rebalancableVehiclesPerZone = new HashMap<>();
		rebalancableVehicles.filter(v -> v.getServiceEndTime() > time + params.getMinServiceTime()).forEach(v -> {
			...
			org.matsim.contrib.my_drt.analysis.zonal.DrtZone zone = zonalSystem.getZoneForLinkId(link.getId());
			if (zone == null) {
				zone = org.matsim.contrib.my_drt.analysis.zonal.DrtZone.createDummyZone("single-vehicle-zone-" + v.getId(), List.of(link), link.getCoord());
			}
	...
	public static Map<org.matsim.contrib.my_drt.analysis.zonal.DrtZone, List<DvrpVehicle>> groupSoonIdleVehicles(DrtZonalSystem zonalSystem,
                                                                                                                 RebalancingParams params, Fleet fleet, double time) {
		Map<org.matsim.contrib.my_drt.analysis.zonal.DrtZone, List<DvrpVehicle>> soonIdleVehiclesPerZone = new HashMap<>();
