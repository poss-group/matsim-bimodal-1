...
package org.matsim.contrib.my_drt.optimizer.depot;
...
public class Depots {
	public static boolean isSwitchingFromStopToStay(DvrpVehicle vehicle) {
		...
		if (!org.matsim.contrib.my_drt.schedule.DrtTaskBaseType.STAY.isBaseTypeOf(currentTask)) {
			return false;
		}

		// previous task was STOP
		int previousTaskIdx = currentTask.getTaskIdx() - 1;
		return (previousTaskIdx >= 0 && DrtTaskBaseType.STOP.isBaseTypeOf(schedule.getTasks().get(previousTaskIdx)));
	}
