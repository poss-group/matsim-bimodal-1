...
//        LOG.setLevel(Level.DEBUG);
     ...
     */
    public double calculate(org.matsim.contrib.my_drt.passenger.DrtRequest drtRequest,
                            org.matsim.contrib.my_drt.optimizer.insertion.InsertionWithDetourData<D> insertion) {
    ...
    double calculatePickupDetourTimeLoss(org.matsim.contrib.my_drt.passenger.DrtRequest drtRequest,
                                         org.matsim.contrib.my_drt.optimizer.insertion.InsertionWithDetourData<D> insertion,
...
//		double pickupEllipseTT = calculatePickupDropoffEllipseCondition(insertion);
    ...
    double calculateDropoffDetourTimeLoss(org.matsim.contrib.my_drt.passenger.DrtRequest drtRequest,
                                          org.matsim.contrib.my_drt.optimizer.insertion.InsertionWithDetourData<D> insertion,
                                          double[] toFromPickupToFromDropoff) {
        org.matsim.contrib.my_drt.optimizer.VehicleData.Entry vEntry = insertion.getVehicleEntry();
        final int pickupIdx = insertion.getPickup().index;
...
//            LOG.debug("drop-off-ellipse is violated");
    ...
    private double calculateReplacedDriveDuration(org.matsim.contrib.my_drt.optimizer.VehicleData.Entry vEntry,
                                                  int insertionIdx) {
    ...
    private boolean isHardConstraintsViolated(
            org.matsim.contrib.my_drt.optimizer.insertion.InsertionWithDetourData<?> insertion,
            double pickupDetourTimeLoss,
            double totalTimeLoss, org.matsim.contrib.my_drt.passenger.DrtRequest drtRequest,
            double[] toFromPickupToFromDropoff) {
        org.matsim.contrib.my_drt.optimizer.VehicleData.Entry vEntry = insertion.getVehicleEntry();
        final int pickupIdx = insertion.getPickup().index;
        final int dropoffIdx = insertion.getDropoff().index;
...
//            lastStopDepartureTime = stop.getDepartureTime();
            if (stop.task.getBeginTime() + pickupDetourTimeLoss > stop.latestArrivalTime //
                    ...
                    || isEllipseConstraintViolated(drtRequest.getFromLink(), stop.getLink(),
                    drtRequest.getToLink()) // request-ellipse constraint
            ) {
...
//        }
        ...
        org.matsim.contrib.my_drt.schedule.DrtStayTask lastTask = (DrtStayTask) Schedules
                .getLastTask(vEntry.vehicle.getSchedule());
    ...
    /**
     * The request constraints are set in {@link org.matsim.contrib.my_drt.passenger.DrtRequest}, which is used by
     * {@link DrtRequestCreator},
