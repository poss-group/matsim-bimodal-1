...
import org.matsim.contrib.my_drt.routing.DefaultDrtRouteUpdater;
import org.matsim.contrib.my_drt.routing.DrtRoute;
import org.matsim.contrib.my_drt.routing.DrtRouteCreator;
import org.matsim.contrib.my_drt.routing.DrtRouteFactory;
import org.matsim.contrib.my_drt.run.DrtConfigGroup;
import org.matsim.contrib.my_drt.run.DrtModeModule;
import org.matsim.contrib.my_drt.schedule.DrtStayTask;
import org.matsim.contrib.my_drt.schedule.DrtTaskBaseType;
...
//        LOG.setLevel(Level.DEBUG);
     ...
     */
    public double calculate(org.matsim.contrib.my_drt.passenger.DrtRequest drtRequest, org.matsim.contrib.my_drt.optimizer.insertion.InsertionWithDetourData<D> insertion) {
        ...
        if (isHardConstraintsViolated(insertion, pickupDetourTimeLoss, totalTimeLoss, drtRequest, toFromPickupToFromDropoff)) {
    ...
    double calculatePickupDetourTimeLoss(org.matsim.contrib.my_drt.passenger.DrtRequest drtRequest, org.matsim.contrib.my_drt.optimizer.insertion.InsertionWithDetourData<D> insertion, double[] toFromPickupToFromDropoff) {
        org.matsim.contrib.my_drt.optimizer.VehicleData.Entry vEntry = insertion.getVehicleEntry();
        ...
        boolean ongoingStopTask = pickupIdx == 0 && schedule.getStatus() == ScheduleStatus.STARTED//
                && DrtTaskBaseType.STOP.isBaseTypeOf(schedule.getCurrentTask());
        ...
        if (isEllipseConstraintViolated(pickupPreviousLink, insertedLink, pickupNextLink)) { // pick-up-ellipse constraint
...
//		double pickupEllipseTT = calculatePickupDropoffEllipseCondition(insertion);
    ...
    double calculateDropoffDetourTimeLoss(org.matsim.contrib.my_drt.passenger.DrtRequest drtRequest, org.matsim.contrib.my_drt.optimizer.insertion.InsertionWithDetourData<D> insertion, double[] toFromPickupToFromDropoff) {
        org.matsim.contrib.my_drt.optimizer.VehicleData.Entry vEntry = insertion.getVehicleEntry();
        final int pickupIdx = insertion.getPickup().index;
        final int dropoffIdx = insertion.getDropoff().index;
        ...
        Link insertedLink = drtRequest.getFromLink();
        if (isEllipseConstraintViolated(dropoffPreviousLink, insertedLink, dropoffNextLink)) { // drop-off-ellipse constraint
//            LOG.debug("drop-off-ellipse is violated");
    ...
    private double calculateReplacedDriveDuration(org.matsim.contrib.my_drt.optimizer.VehicleData.Entry vEntry, int insertionIdx) {
    ...
    private static org.matsim.contrib.my_drt.run.DrtModeModule.DrtRouteCreatorProvider drtRouteCreatorProvider;
    ...
    private boolean isHardConstraintsViolated(org.matsim.contrib.my_drt.optimizer.insertion.InsertionWithDetourData<?> insertion, double pickupDetourTimeLoss,
                                              double totalTimeLoss, org.matsim.contrib.my_drt.passenger.DrtRequest drtRequest, double[] toFromPickupToFromDropoff) {
        org.matsim.contrib.my_drt.optimizer.VehicleData.Entry vEntry = insertion.getVehicleEntry();
        final int pickupIdx = insertion.getPickup().index;
        final int dropoffIdx = insertion.getDropoff().index;
        ...
        for (int s = pickupIdx; s < dropoffIdx; s++) {
            org.matsim.contrib.my_drt.optimizer.VehicleData.Stop stop = vEntry.stops.get(s);
            // all stops after pickup but still before dropoff are delayed by pickupDetourTimeLoss
            detourTravelTime += stop.getDepartureTime() - lastStopDepartureTime;
            lastStopDepartureTime = stop.getDepartureTime();
            if (stop.task.getBeginTime() + pickupDetourTimeLoss > stop.latestArrivalTime //
                    ...
                    || isEllipseConstraintViolated(drtRequest.getFromLink(), stop.getLink(), drtRequest.getToLink()) // request-ellipse constraint
            ) {
//                LOG.debug("request-ellipse could have been violated");
                return true;
            }
        }
//        if (totalTravelTime > drtRequest.directTime) {
//            ...
//        }
...
//            LOG.warn("relative_delivery_delay_constraint violated");
        ...
        org.matsim.contrib.my_drt.schedule.DrtStayTask lastTask = (DrtStayTask) Schedules.getLastTask(vEntry.vehicle.getSchedule());
    ...
    /**
     * The request constraints are set in {@link org.matsim.contrib.my_drt.passenger.DrtRequest}, which is used by {@link DrtRequestCreator},
    ...
    private boolean isEllipseConstraintViolated(Link previousLink, Link insertedLink, Link nextLink) {
        ...
        return detourDistance > DETOUR_DELTA * directDistance;
    }
}
