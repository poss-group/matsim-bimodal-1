...
public class DrtTripsAnalyser {
	...
	public static void analyseDetours(Network network, List<DrtTrip> trips, DrtConfigGroup drtCfg, String fileName,
		...
		if (createGraphs) {
			final JFreeChart chart = org.matsim.contrib.my_drt.analysis.DensityScatterPlots.createPlot("Travelled Distances", "travelled distance [m]",
					"unshared ride distance [m]", distances);
			ChartSaveUtils.saveAsPNG(chart, fileName + "_distancePlot", 1500, 1500);

			final JFreeChart chart2 = org.matsim.contrib.my_drt.analysis.DensityScatterPlots.createPlot("Travel Times", "travel time [s]",
	...
	public static void writeVehicleDistances(
            Map<Id<Vehicle>, org.matsim.contrib.my_drt.analysis.DrtPassengerAndVehicleStats.VehicleState> vehicleDistances, String iterationFilename) {
		...
		try {
			bw.write(header);
			bw.newLine();
			for (Entry<Id<Vehicle>, org.matsim.contrib.my_drt.analysis.DrtPassengerAndVehicleStats.VehicleState> e : vehicleDistances.entrySet()) {
	 ...
	 * @return
	 */
	public static String summarizeVehicles(Map<Id<Vehicle>, org.matsim.contrib.my_drt.analysis.DrtPassengerAndVehicleStats.VehicleState> vehicleDistances,
			String del) {
		DecimalFormat format = new DecimalFormat();
		format.setDecimalFormatSymbols(new DecimalFormatSymbols(Locale.US));
		format.setMinimumIntegerDigits(1);
		...
		for (org.matsim.contrib.my_drt.analysis.DrtPassengerAndVehicleStats.VehicleState state : vehicleDistances.values()) {
	...
	public static double getTotalDistance(Map<Id<Vehicle>, org.matsim.contrib.my_drt.analysis.DrtPassengerAndVehicleStats.VehicleState> vehicleDistances) {
		DescriptiveStatistics driven = new DescriptiveStatistics();
		for (org.matsim.contrib.my_drt.analysis.DrtPassengerAndVehicleStats.VehicleState state : vehicleDistances.values()) {
			driven.addValue(state.totalDistance);
		}
	...
	public static String summarizeDetailedOccupancyStats(
            Map<Id<Vehicle>, org.matsim.contrib.my_drt.analysis.DrtPassengerAndVehicleStats.VehicleState> vehicleDistances, String del, int maxcap) {
		DecimalFormat format = new DecimalFormat();
		format.setDecimalFormatSymbols(new DecimalFormatSymbols(Locale.US));
		format.setMinimumIntegerDigits(1);
		format.setMaximumFractionDigits(2);
