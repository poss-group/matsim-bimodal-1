...
package org.matsim.contrib.my_drt.scheduler;

import com.google.inject.name.Named;
import org.matsim.api.core.v01.network.Link;
import org.matsim.contrib.my_drt.optimizer.insertion.InsertionWithDetourData;
...
public class RequestInsertionScheduler {
	...
	private final org.matsim.contrib.my_drt.schedule.DrtTaskFactory taskFactory;

	public RequestInsertionScheduler(DrtConfigGroup drtCfg, Fleet fleet, MobsimTimer timer,
                                     @Named(DvrpTravelTimeModule.DVRP_ESTIMATED) TravelTime travelTime,
                                     ScheduleTimingUpdater scheduleTimingUpdater, org.matsim.contrib.my_drt.schedule.DrtTaskFactory taskFactory) {
		this.fleet = fleet;
	...
	public void scheduleRequest(org.matsim.contrib.my_drt.passenger.DrtRequest request, InsertionWithDetourData<PathData> insertion) {
		insertPickup(request, insertion);
		insertDropoff(request, insertion);
	}

	private void insertPickup(org.matsim.contrib.my_drt.passenger.DrtRequest request, InsertionWithDetourData<PathData> insertion) {
		org.matsim.contrib.my_drt.optimizer.VehicleData.Entry vehicleEntry = insertion.getVehicleEntry();
		Schedule schedule = vehicleEntry.vehicle.getSchedule();
		List<org.matsim.contrib.my_drt.optimizer.VehicleData.Stop> stops = vehicleEntry.stops;
		...
		if (pickupIdx == 0 && scheduleStatus != ScheduleStatus.PLANNED && org.matsim.contrib.my_drt.schedule.DrtTaskBaseType.DRIVE.isBaseTypeOf(currentTask)) {
			...
			if (diversion != null) { // divert vehicle
				...
				if (request.getFromLink() != vehicleEntry.start.link) { // add a new drive task
					VrpPathWithTravelData vrpPath = VrpPaths.createPath(vehicleEntry.start.link, request.getFromLink(),
							vehicleEntry.start.time, insertion.getDetourToPickup(), travelTime);
					beforePickupTask = taskFactory.createDriveTask(vehicleEntry.vehicle, vrpPath, org.matsim.contrib.my_drt.schedule.DrtDriveTask.TYPE);
			...
			org.matsim.contrib.my_drt.schedule.DrtStayTask stayTask = null;
			org.matsim.contrib.my_drt.schedule.DrtStopTask stopTask = null;
			if (pickupIdx == 0) {
				if (scheduleStatus == ScheduleStatus.PLANNED) {// PLANNED schedule
					stayTask = (org.matsim.contrib.my_drt.schedule.DrtStayTask)schedule.getTasks().get(0);
					stayTask.setEndTime(stayTask.getBeginTime());// could get later removed with ScheduleTimingUpdater
				} else if (org.matsim.contrib.my_drt.schedule.DrtTaskBaseType.STAY.isBaseTypeOf(currentTask)) {
					stayTask = (org.matsim.contrib.my_drt.schedule.DrtStayTask)currentTask; // ongoing stay task
					...
					stopTask = (org.matsim.contrib.my_drt.schedule.DrtStopTask)currentTask; // ongoing stop task
				}
			} else {
			...
			if (stopTask != null && request.getFromLink() == stopTask.getLink()) { // no detour; no new stop task
				...
				if (pickupIdx == dropoffIdx) {
					...
					if (pickupIdx < stops.size()) {// there is at least one following stop
						org.matsim.contrib.my_drt.schedule.DrtStopTask nextStopTask = stops.get(pickupIdx).task;
					...
					Task driveFromPickupTask = taskFactory.createDriveTask(vehicleEntry.vehicle, vrpPath,
							org.matsim.contrib.my_drt.schedule.DrtDriveTask.TYPE);
		...
		org.matsim.contrib.my_drt.schedule.DrtStopTask pickupStopTask = taskFactory.createStopTask(vehicleEntry.vehicle, startTime,
		...
		Task driveFromPickupTask = taskFactory.createDriveTask(vehicleEntry.vehicle, vrpPath, org.matsim.contrib.my_drt.schedule.DrtDriveTask.TYPE);
	...
	private void insertDropoff(DrtRequest request, InsertionWithDetourData<PathData> insertion) {
		...
		if (pickupIdx == dropoffIdx) { // no drive to dropoff
			...
			org.matsim.contrib.my_drt.schedule.DrtStopTask stopTask = stops.get(dropoffIdx - 1).task;
			if (request.getToLink() == stopTask.getLink()) { // no detour; no new stop task
				...
				if (dropoffIdx < stops.size()) {
					org.matsim.contrib.my_drt.schedule.DrtStopTask nextStopTask = stops.get(dropoffIdx).task;
				...
				driveToDropoffTask = taskFactory.createDriveTask(vehicleEntry.vehicle, vrpPath, org.matsim.contrib.my_drt.schedule.DrtDriveTask.TYPE);
		...
		org.matsim.contrib.my_drt.schedule.DrtStopTask dropoffStopTask = taskFactory.createStopTask(vehicleEntry.vehicle, startTime,
		...
		if (dropoffIdx == stops.size()) {// bus stays at dropoff
			if (taskIdx + 2 == schedule.getTaskCount()) {// remove stay task from the end of schedule,
				org.matsim.contrib.my_drt.schedule.DrtStayTask oldStayTask = (org.matsim.contrib.my_drt.schedule.DrtStayTask)schedule.getTasks().get(taskIdx + 1);
